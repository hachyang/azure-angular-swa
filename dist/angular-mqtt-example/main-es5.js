...
var AppComponent = /** @class */ (function () {
    function AppComponent(_mqttService, http) {
        var _this = this;
        ...
        this.onInputFocus = function (event) {
            _this.inputName = event.target.id;
            _this.show = true;
            /*
             ...
             */
            console.log("Focused input", _this.inputName);
        ...
        this.onChange = function (input) {
            _this.inputs[_this.inputName] = input;
             ...
             */
            var caretPosition = _this.keyboard.caretPosition;
            if (caretPosition !== null)
                _this.setInputCaretPosition(document.querySelector("#" + _this.inputName), caretPosition);
        ...
        */
        ...
        node_red_contrib_uibuilder_front_end_src_uibuilderfe__WEBPACK_IMPORTED_MODULE_4___default.a.onChange('msg', function (newVal) {
            _this.state.msgRecvd = newVal;
            _this.subscribeDefaultTopic();
            console.info('[uibuilder.onChange] msg received from Node-RED server:', newVal);
        });
        // As we receive new messages, we get an updated count as well
        node_red_contrib_uibuilder_front_end_src_uibuilderfe__WEBPACK_IMPORTED_MODULE_4___default.a.onChange('msgsReceived', function (newVal) {
            console.info('[uibuilder.onChange] Updated count of received msgs:', newVal);
            _this.state.msgsReceived = newVal;
        });
        // If we receive a control message from Node-RED, we can get the new data here - we pass it to a Vue variable
        node_red_contrib_uibuilder_front_end_src_uibuilderfe__WEBPACK_IMPORTED_MODULE_4___default.a.onChange('ctrlMsg', function (newVal) {
            console.info('[uibuilder.onChange:ctrlMsg] CONTROL msg received from Node-RED server:', newVal);
            _this.state.msgCtrl = newVal;
        });
        // Updated count of control messages received
        node_red_contrib_uibuilder_front_end_src_uibuilderfe__WEBPACK_IMPORTED_MODULE_4___default.a.onChange('msgsCtrl', function (newVal) {
            console.info('[uibuilder.onChange:msgsCtrl] Updated count of received CONTROL msgs:', newVal);
            _this.state.msgsControl = newVal;
        });
        //#endregion ---- End of Trace Received Messages ---- //
        //#region ---- Trace Sent Messages ---- //
        // You probably only need these to help you understand the order of processing //
        // If a message is sent back to Node-RED, we can grab a copy here if we want to
        node_red_contrib_uibuilder_front_end_src_uibuilderfe__WEBPACK_IMPORTED_MODULE_4___default.a.onChange('sentMsg', function (newVal) {
            console.info('[uibuilder.onChange:sentMsg] msg sent to Node-RED server:', newVal);
            _this.state.msgSent = newVal;
        });
        // Updated count of sent messages
        node_red_contrib_uibuilder_front_end_src_uibuilderfe__WEBPACK_IMPORTED_MODULE_4___default.a.onChange('msgsSent', function (newVal) {
            console.info('[uibuilder.onChange:msgsSent] Updated count of msgs sent:', newVal);
            _this.state.msgsSent = newVal;
        });
        // If we send a control message to Node-RED, we can get a copy of it here
        node_red_contrib_uibuilder_front_end_src_uibuilderfe__WEBPACK_IMPORTED_MODULE_4___default.a.onChange('sentCtrlMsg', function (newVal) {
            console.info('[uibuilder.onChange:sentCtrlMsg] Control message sent to Node-RED server:', newVal);
            _this.state.msgCtrlSent = newVal;
        });
        // And we can get an updated count
        node_red_contrib_uibuilder_front_end_src_uibuilderfe__WEBPACK_IMPORTED_MODULE_4___default.a.onChange('msgsSentCtrl', function (newVal) {
            console.info('[uibuilder.onChange:msgsSentCtrl] Updated count of CONTROL msgs sent:', newVal);
            _this.state.msgsCtrlSent = newVal;
        });
        //#endregion ---- End of Trace Sent Messages ---- //
        // If Socket.IO connects/disconnects, we get true/false here
        node_red_contrib_uibuilder_front_end_src_uibuilderfe__WEBPACK_IMPORTED_MODULE_4___default.a.onChange('ioConnected', function (newVal) {
            console.info('[uibuilder.onChange:ioConnected] Socket.IO Connection Status Changed to:', newVal);
            _this.state.socketConnectedState = newVal;
        });
        // If Server Time Offset changes
        node_red_contrib_uibuilder_front_end_src_uibuilderfe__WEBPACK_IMPORTED_MODULE_4___default.a.onChange('serverTimeOffset', function (newVal) {
            console.info('[uibuilder.onChange:serverTimeOffset] Offset of time between the browser and the server has changed to:', newVal);
            _this.state.serverTimeOffset = newVal;
        });
    ...
    AppComponent.prototype.ngAfterViewInit = function () {
        ...
        */
        ...
        this.keyboard = new simple_keyboard__WEBPACK_IMPORTED_MODULE_5___default.a({
            ...
            onKeyPress: function (button) { return _this.onKeyPress(button); },
            preventMouseDownDefault: true // If you want to keep focus on input
        });
        /**
    ...
    */
    AppComponent.prototype.subscribeDefaultTopic = function () {
        //this.state.msgRecvd = {topic: "hello", payload:"baba",_socketId:"kkkop",_msgid: "977yhh"};
        var msgData = JSON.parse(JSON.stringify(this.state.msgRecvd));
    ...
    AppComponent.prototype.subscribeNewTopic = function () {
        var _this = this;
        console.log('inside subscribe new topic');
        this.subscription = this._mqttService.observe(this.inputs.topicname).subscribe(function (message) {
            //this.msg = message;
            console.log('msg: ', message);
            if (message.topic == _this.unitsGetTopic) {
                _this.inputs.units = message.payload.toString();
            }
            else {
                _this.logMsg('Message: ' + message.payload.toString() + '<br> for topic: ' + message.topic);
            }
        });
    ...
    AppComponent.prototype.sendmsg = function () {
        ...
        var apiUrl = '/api/HttpTrigger?Module=' + this.inputs.topicname + '&Device=' + this.inputs.units + '&Location=' + this.inputs.msg;
        this.logMsg(apiUrl);
        this.http.get(apiUrl).subscribe(function (resp) { return _this.logMsg(resp.text); });
        this.inputs.msg = '';
    };
    AppComponent.prototype.changeUnits = function () {
    ...
    // }
    AppComponent.prototype.logMsg = function (message) {
